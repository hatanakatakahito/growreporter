...
function addAIAndMemoFooter(slide, aiData, memos, y, maxH) {
  ...
  if (aiData?.summary) {
    ...
    if (aiText) {
      slide.addText([
        { text: 'AI分析: ', options: { bold: true, color: COLORS.accent, fontSize: 7, fontFace: FONT_FACE } },
        { text: aiText, options: { color: COLORS.subText, fontSize: 7, fontFace: FONT_FACE } },
      ], {
        x: MARGIN_X, y: currentY, w: CONTENT_W, h: Math.max(maxH * 0.6, 0.3),
        valign: 'top', wrap: true, shrinkText: true,
      });
      currentY += Math.max(maxH * 0.6, 0.3);
    }
  }
  ...
  if (memos && memos.length > 0) {
    ...
    const memoContent = [
      { text: 'メモ: ', options: { bold: true, color: COLORS.memoLabel, fontSize: 7, fontFace: FONT_FACE } },
      { text: memoTexts.join('\n'), options: { color: COLORS.subText, fontSize: 7, fontFace: FONT_FACE } },
    ];

    const remainH = Math.max(maxH - (currentY - y), 0.2);
    slide.addText(memoContent, {
      x: MARGIN_X, y: currentY, w: CONTENT_W, h: remainH,
      valign: 'top', wrap: true, shrinkText: true,
    });
  }
}
...
// 1. 表紙
function createCoverSlide(pptx, siteName, siteUrl, dateRange, logoBase64) {
  ...
  if (logoBase64) {
    slide.addImage({
      data: logoBase64,
      x: 3.5, y: 0.8, w: 4, h: 0.96,
    });
  }

  // タイトル
  slide.addText('分析レポート', {
    x: MARGIN_X, y: logoBase64 ? 2.0 : 1.5, w: CONTENT_W, h: 0.8,
    fontSize: 36, fontFace: FONT_FACE, bold: true,
  ...
  const tableRows = infoRows.map(([label, val]) => [
    { text: label, options: { bold: true, color: COLORS.subText, fontSize: 12, fontFace: FONT_FACE, align: 'right', valign: 'middle', fill: { color: COLORS.lightGray }, border: { type: 'solid', pt: 0.5, color: COLORS.border } } },
    { text: val, options: { color: COLORS.dark, fontSize: 12, fontFace: FONT_FACE, align: 'left', valign: 'middle', fill: { color: COLORS.white }, border: { type: 'solid', pt: 0.5, color: COLORS.border } } },
  ]);

  slide.addTable(tableRows, {
    x: 2.5, y: logoBase64 ? 3.3 : 3.5, w: 6, colW: [1.8, 4.2],
    rowH: 0.5,
    border: { type: 'solid', pt: 0.5, color: COLORS.border },
  });
...
function createSummarySlide(pptx, dashboard, kpiSettings, aiData, memos) {
  const slide = pptx.addSlide();
  ...
  if (dashboard?.metrics) {
    ...
    const metricsData = [
      ['セッション数', formatNumber(m.sessions)],
      ['ユーザー数', formatNumber(m.totalUsers)],
      ['新規ユーザー数', formatNumber(m.newUsers)],
      ['ページビュー数', formatNumber(m.pageViews)],
      ['エンゲージメント率', fmtPct(m.engagementRate)],
      ['コンバージョン数', formatNumber(m.conversions)],
    ];

    // GSC指標
    if (m.clicks || m.impressions) {
      metricsData.push(
        ['クリック数（GSC）', formatNumber(m.clicks)],
        ['表示回数（GSC）', formatNumber(m.impressions)],
        ['CTR（GSC）', fmtPct(m.ctr)],
    ...
    for (let i = 0; i < metricsData.length; i += 2) {
      ...
      // 左カード
      row.push(
        { text: metricsData[i][0], options: { bold: true, color: COLORS.subText, fontSize: 9, fontFace: FONT_FACE, align: 'left', valign: 'middle', fill: { color: COLORS.lightGray }, border: { type: 'solid', pt: 0.5, color: COLORS.border } } },
        { text: metricsData[i][1], options: { bold: true, color: COLORS.primary, fontSize: 14, fontFace: FONT_FACE, align: 'right', valign: 'middle', fill: { color: COLORS.white }, border: { type: 'solid', pt: 0.5, color: COLORS.border } } },
      );
      // 右カード
      if (i + 1 < metricsData.length) {
        row.push(
          { text: metricsData[i + 1][0], options: { bold: true, color: COLORS.subText, fontSize: 9, fontFace: FONT_FACE, align: 'left', valign: 'middle', fill: { color: COLORS.lightGray }, border: { type: 'solid', pt: 0.5, color: COLORS.border } } },
          { text: metricsData[i + 1][1], options: { bold: true, color: COLORS.primary, fontSize: 14, fontFace: FONT_FACE, align: 'right', valign: 'middle', fill: { color: COLORS.white }, border: { type: 'solid', pt: 0.5, color: COLORS.border } } },
        );
      } else {
        row.push(
    ...
    // KPI達成状況
    let kpiEndY = CONTENT_Y + tableRows.length * 0.5 + 0.2;
    if (kpiSettings?.kpiList && kpiSettings.kpiList.length > 0) {
      ...
      if (activeKpis.length > 0) {
        ...
        kpiEndY += (kpiRows.length + 1) * 0.3 + 0.1;
      }
    }

    // AI分析+メモ
    const aiY = Math.max(kpiEndY, FOOTER_Y - 1.0);
    addAIAndMemoFooter(slide, aiData, memos, aiY, FOOTER_Y - aiY - 0.05);
  }
...
// 月別（13ヶ月推移）
function createMonthlySlide(pptx, monthlyData, aiData, memos) {
  ...
  const hasAI = !!aiData?.summary;
  const hasMemos = memos && memos.length > 0;
  const layout = calcLayout(monthlyData.length, hasAI, hasMemos);

  // 折れ線チャート
  const labels = monthlyData.map(d => fmtYearMonth(d.month || d.label || d.yearMonth));
  const chartData = [
    { name: 'ユーザー', labels, values: monthlyData.map(d => fmtNum(d.users || d.totalUsers)) },
    { name: 'セッション', labels, values: monthlyData.map(d => fmtNum(d.sessions)) },
    { name: 'PV', labels, values: monthlyData.map(d => fmtNum(d.pageViews || d.screenPageViews)) },
    { name: 'CV', labels, values: monthlyData.map(d => fmtNum(d.conversions || d.totalConversions || 0)) },
  ];
  ...
  slide.addChart(pptx.charts.LINE, chartData, {
    ...
    chartColors: ['3b82f6', 'f59e0b', '8b5cf6', 'ef4444'],
  ...
  addAIAndMemoFooter(slide, aiData, memos, layout.aiY, layout.aiHeight);
  addSlideFooter(slide);
}

// 日別推移
function createDailySlide(pptx, dailyData, aiData, memos) {
  if (!dailyData?.rows || dailyData.rows.length === 0) return;

  const dataRows = dailyData.rows;
  const hasAI = !!aiData?.summary;
  const hasMemos = memos && memos.length > 0;
  ...
  addAIAndMemoFooter(slide2, aiData, memos, tableLayout.aiY, tableLayout.aiHeight);
...
function createPagesSlide(pptx, pages, aiData, memos) {
  ...
  slide.addChart(pptx.charts.BAR, chartData, {
    ...
    catAxisLabelFontSize: 5, valAxisLabelFontSize: 7,
...
function createConversionsSlide(pptx, conversions, aiData, memos) {
  ...
  const convData = conversions.data;
  const hasAI = !!aiData?.summary;
  const hasMemos = memos && memos.length > 0;
...
export async function downloadAnalysisPptx(allData, siteName, dateRange) {
  ...
  // 1. 表紙
  createCoverSlide(pptx, siteName, allData.siteUrl, dateRange, logoBase64);

  // 2. 全体サマリー（主要指標 + KPI予実 + コンバージョン内訳）
  createSummarySlide(pptx, allData.summaryMetrics, allData.kpiSettings, ai['analysis/summary'], memos['analysis/summary']);

  // セクション2: トレンド分析
  createSectionDivider(pptx, 'トレンド分析');
  createMonthlySlide(pptx, allData.monthlyData, ai['analysis/month'], memos['analysis/month']);
  createDailySlide(pptx, allData.daily, ai['analysis/day'], memos['analysis/day']);
