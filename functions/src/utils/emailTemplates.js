...
// セキュリティ (Phase 4-A-5): メールテンプレ XSS 対策
// ユーザー登録名・会社名・サイト名・メッセージ等を HTML に埋め込む際は必ず escape する。
...
export function getBusinessUpsellHtml() {
  return `
    <div style="background: linear-gradient(135deg, #f87171 0%, #ec4899 100%); border-radius: 12px; padding: 24px; margin-top: 32px; text-align: center;">
      ...
      <p style="color: rgba(255,255,255,0.9); font-size: 13px; margin: 0 0 16px;">ビジネスプランなら、この数値の背景や改善策をAIが自動で分析します。</p>
      <a href="https://growgroupreporter.web.app/plan-info" style="display: inline-block; background: white; color: #ec4899; font-weight: bold; padding: 10px 24px; border-radius: 8px; text-decoration: none; font-size: 14px;">ビジネスプランを詳しく見る</a>
    </div>`;
}
...
export function generateInvitationEmail(data) {
  ...
  // XSS 対策: HTML 文脈で展開する変数は escape 済の値を使う
          ...
          <tr>
            <td style="padding: 40px 30px;">
              ...
              <p style="margin: 20px 0; color: #374151; font-size: 14px; line-height: 1.6;">
                招待を承認すると、${companyNameH} の全サイトのデータにアクセスできるようになります。
              </p>
        ...
        <table cellpadding="0" cellspacing="0" border="0" width="600" style="background-color: #ffffff; border-radius: 8px; padding: 30px;">
          <tr>
            <td>
              ...
              <p style="margin: 0 0 15px 0; color: #1f2937;">${memberNameH} さん、</p>
              <p style="margin: 0 0 15px 0; color: #1f2937;">
                ${companyNameH} のメンバーから削除されました。今後、このアカウントのサイトにはアクセスできません。
              </p>
              <p style="margin: 0; color: #374151; font-size: 14px;">
        ...
        <table cellpadding="0" cellspacing="0" border="0" width="600" style="background-color: #ffffff; border-radius: 8px; padding: 30px;">
          <tr>
            <td>
              ...
              <p style="margin: 0 0 15px 0; color: #1f2937;">${newOwnerNameH} さん、</p>
              <p style="margin: 0 0 15px 0; color: #1f2937;">
                ${previousOwnerNameH} さんから、<strong>${companyNameH}</strong> のオーナー権限が譲渡されました。
              </p>
...
export function generateAdminCreatedAccountEmail(data) {
  ...
  // XSS 対策: name/email/password を HTML 内で表示する際に必ず escape
  const displayNameH = escapeHtml(displayName);
  const emailH = escapeHtml(email);
  const passwordH = escapeHtml(password);
  const loginUrlH = escapeHtmlAndValidateUrl(loginUrl);
          ...
          <tr>
            <td style="padding: 40px 30px;">
                    ...
                    <table cellpadding="0" cellspacing="0" border="0" width="100%">
                      <tr>
                        ...
                        <td style="padding: 8px 0; color: #1f2937; font-size: 14px; font-weight: 600;">
                          <a href="${loginUrlH}" style="color: #3758F9; text-decoration: none;">${loginUrlH}</a>
                        </td>
                      </tr>
                      <tr>
                        <td style="padding: 8px 0; border-top: 1px solid #e5e7eb; color: #374151; font-size: 13px; vertical-align: top;">メールアドレス</td>
                        <td style="padding: 8px 0; border-top: 1px solid #e5e7eb; color: #1f2937; font-size: 14px; font-weight: 600;">${emailH}</td>
                      </tr>
                      <tr>
                        <td style="padding: 8px 0; border-top: 1px solid #e5e7eb; color: #374151; font-size: 13px; vertical-align: top;">パスワード</td>
                        <td style="padding: 8px 0; border-top: 1px solid #e5e7eb; color: #1f2937; font-size: 14px; font-weight: 600; letter-spacing: 1px;">${passwordH}</td>
                      </tr>
                    </table>
                  </td>
                </tr>
              ...
              <table cellpadding="0" cellspacing="0" border="0" width="100%" style="margin: 30px 0;">
                <tr>
                  <td align="center">
                    <a href="${loginUrlH}" style="display: inline-block; background-color: #3758F9; color: #ffffff; text-decoration: none; padding: 14px 32px; border-radius: 6px; font-size: 16px; font-weight: 600;">
                      ログインする
                    </a>
                  </td>
                </tr>
...
/**
 * アカウント情報メール（admin が任意のタイミングで送信）
 *
 * §16: admin がサイレントでユーザーを作成 → サイト登録など準備完了 →
 * このメールで顧客にアカウント情報を通知。
 *
 * パスワードは生で持っていないため、Firebase Auth のパスワードリセットリンクを
 * 添えて顧客自身に設定してもらう。
 */
export function generateAccountCredentialsEmail(data) {
  const { userName, email, resetLink } = data;
  const displayName = userName || 'ユーザー';
  const loginUrl = 'https://grow-reporter.com/login';

  const subject = '【グローレポータ】アカウント情報のご案内';
      ...
      <td align="center">
        <table cellpadding="0" cellspacing="0" border="0" width="600" style="background-color: #ffffff; border-radius: 8px; overflow: hidden; box-shadow: 0 4px 6px rgba(0, 0, 0, 0.1);">
          ...
          <tr>
            <td style="background-color: #3758F9; padding: 30px; text-align: center;">
              <h1 style="margin: 0; color: #ffffff; font-size: 24px; font-weight: 700;">グローレポータ</h1>
              <p style="margin: 10px 0 0 0; color: #e0e7ff; font-size: 14px;">アカウント情報のご案内</p>
            </td>
          </tr>
          ...
          <tr>
            <td style="padding: 40px 30px;">
              ...
              <p style="margin: 0 0 20px 0; color: #1f2937; font-size: 16px; line-height: 1.6;">
                グローレポータのアカウントが発行されました。<br>
                以下の手順でパスワードを設定してログインしてください。
              </p>

              <table cellpadding="0" cellspacing="0" border="0" width="100%" style="background-color: #f9fafb; border-radius: 8px; margin: 24px 0;">
                <tr>
                  <td style="padding: 24px;">
                    <p style="margin: 0 0 12px 0; color: #374151; font-size: 13px;">ログインメールアドレス</p>
                    <p style="margin: 0; color: #1f2937; font-size: 16px; font-weight: 600;">${email}</p>
                  </td>
                </tr>
              </table>

              <p style="margin: 20px 0 12px 0; color: #1f2937; font-size: 15px; font-weight: 600;">
                パスワード設定はこちら
              </p>
              ...
              <p style="margin: 20px 0 0 0; color: #6b7280; font-size: 13px; line-height: 1.6;">
                ※ 上記リンクの有効期限は 72 時間です。期限切れの場合は担当者までご連絡ください。<br>
