...
- 専門用語はできるだけ避け、やさしい日本語で書く。どうしても使う場合は括弧で補足。例: お問い合わせ率（CVR）、1ページだけ見て離脱する割合（直帰率）
...
- 季節性やイベント影響の可能性にも言及
- 専門用語はできるだけ避け、やさしい日本語で書く。どうしても使う場合は括弧で補足。例: お問い合わせ率（CVR）、閲覧の活発さ（エンゲージメント率）
- 数値は実数値またはパーセンテージのみ（「ポイント」表記は禁止）
- 改善提案は含めない
...
`;
  ...
  if (metrics.scrapingData && metrics.scrapingData.pages && metrics.scrapingData.pages.length > 0) {
    ...
    pages.slice(0, 30).forEach((page, index) => {
      ...
      if (page.hasForm) {
        // フォームフィールドのラベル一覧を表示（項目数はデータ取得の制約で正確でない場合があるため記載しない）
      ...
      // フォーム詳細（deepPageScraper由来: purpose, fields, submitText）
...
- 150-200文字程度の自然な文章（段落形式）
- 前置きや挨拶は一切不要、分析内容から直接始める
- 専門用語はできるだけ避け、やさしい日本語で書く。どうしても使う場合は括弧で補足。例: 検索からの流入（オーガニック検索）、他サイトからのリンク（リファラル）
- 数値は実数値またはパーセンテージのみ（「ポイント」表記は禁止）
- 改善提案は含めない
...
- 150-200文字程度の自然な文章（段落形式）
- 前置きや挨拶は一切不要、分析内容から直接始める
- 専門用語はできるだけ避け、やさしい日本語で書く。どうしても使う場合は括弧で補足。例: 最初に見られるページ（ランディングページ）
- 数値は実数値またはパーセンテージのみ（「ポイント」表記は禁止）
- 改善提案は含めない
...
- 150-200文字程度の自然な文章（段落形式）
- 前置きや挨拶は一切不要、分析内容から直接始める
- 専門用語はできるだけ避け、やさしい日本語で書く。どうしても使う場合は括弧で補足。例: ページの閲覧数（PV）
- 数値は実数値またはパーセンテージのみ（「ポイント」表記は禁止）
- 改善提案は含めない
...
- 150-200文字程度の自然な文章（段落形式）
- 前置きや挨拶は一切不要、分析内容から直接始める
- 専門用語はできるだけ避け、やさしい日本語で書く。どうしても使う場合は括弧で補足。例: クリック率（CTR）、検索結果での表示回数（インプレッション）
- 数値は実数値またはパーセンテージのみ（「ポイント」表記は禁止）
- 改善提案は含めない
...
- 150-200文字程度の自然な文章（段落形式）
- 前置きや挨拶は一切不要、分析内容から直接始める
- 専門用語はできるだけ避け、やさしい日本語で書く。どうしても使う場合は括弧で補足。例: お問い合わせや資料請求などの成果（CV）
- 数値は実数値またはパーセンテージのみ（「ポイント」表記は禁止）
- 改善提案は含めない
