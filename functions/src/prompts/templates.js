...
※ 上記タスク1のデータのうち、特に「チャネル別データ」「流入キーワード」「ランディングページ」「参照元」を重点的に深掘りすること。
※ スクレイピングデータ（各ページのmeta description、title、h1構成）もSEO改善の根拠として活用すること。

※【重要】集客に直接関係しない施策（デザイン改善、フォーム最適化、表示速度改善等）は提案しないこと。あくまで「サイトへの訪問者数を増やす」施策のみ出すこと。

✓ チャネル別セッションデータを精査し、Organic Search / Paid Search / Social / Referral / Direct それぞれの強弱を特定
✓ 弱いチャネルに対する具体的な強化策を提示（「SNSが弱い」→「Instagram/X/YouTubeのどれをどう活用するか」まで踏み込む）
✓ ランディングページ別の流入数・直帰率・CVRを分析し、改善すべきLPを特定
✓ 流入キーワードの順位・クリック率を分析し、順位改善の余地があるKWを特定
✓ SEO施策: 順位が惜しい（4-20位）キーワードのコンテンツ強化、メタタグ最適化、内部リンク構造改善
✓ SNS・YouTube連携: サイトコンテンツとSNS/動画の相互導線、シェアされやすいコンテンツの提案
✓ 広告施策: リスティング広告のKW選定、SNS広告のターゲティング改善、リマーケティング活用
✓ 新規流入経路の開拓: プレスリリース、業界メディア掲載、オフライン連携（QRコード等）
✓ 各施策に対して「どのチャネルの、何の数値を、どれくらい改善するか」の目標を明示
\n`;
...
※ 上記タスク1のデータのうち、特に「コンバージョンデータ」「逆算フロー（CV直前のページ遷移）」「ページフロー」を最優先で分析すること。
※ KPI設定がある場合は目標達成状況を評価し、未達の指標に対する改善策を提案すること。
※ スクレイピングデータからフォームページの構造（入力項目数、CTA文言等）を確認すること。
※【重要】コンバージョンに直接関係しない施策（SEO改善、ブランディング、表示速度改善等）は提案しないこと。あくまで「成果数・成果率を上げる」施策のみ出すこと。

${hasConversionSettings
  ? '✓ 設定されているコンバージョン目標・KPI目標値に対する達成状況を分析し、未達の目標に対する改善策を優先提案\n'
  : '✓ コンバージョン設定が未設定のため、まず設定を促す旨を明記すること\n'}
✓ コンバージョン一覧データを精査: どのCVイベントが多い/少ないか、CVR（成果率）の推移を確認
✓ 逆算フロー（CV直前のページ遷移）を分析し、成果につながる導線と離脱ポイントを特定
✓ CVに至るユーザーの行動パターン（流入元→閲覧ページ→CV）を分析し、成功パターンを強化
✓ EFO（Entry Form Optimization）: フォームの入力項目数、必須/任意の最適化、ステップ分割、入力補助
✓ フォームページへの導線改善: CTAボタンの配置・文言・色・サイズの最適化、各ページからフォームへの誘導強化
✓ フォームページの離脱率を下げる施策: 入力中の不安解消（プライバシー表示、所要時間表示等）
✓ マイクロコンバージョンの設計: 資料DL、メルマガ登録など中間CVの追加でファネルを補強
✓ 各施策に対して「CVRをどれくらい改善するか」の期待値を数値で明示
\n`;
    } else if (improvementFocus === 'ブランディングの向上') {
      improvementFocusLine += `【ブランディング向上の重点ポイント】
※ スクレイピングデータ（各ページのスクリーンショット・meta・h1・コンテンツ構造）を最重点でチェックすること。
※ サイトの実際の見た目・デザイン・写真・配色から、その会社・サービス・製品の「らしさ」が表現できているかを評価すること。
※ サイトコンテキスト（業種・サイトの目的・サイトの種類）を踏まえた「あるべきブランド像」と現状のギャップを指摘すること。
※【重要】ブランディングに直接関係しない技術的改善（表示速度改善、alt属性設定、h1タグ修正、構造化データ等）は提案しないこと。あくまでビジュアル・デザイン・ブランド表現に関する提案のみ出すこと。

✓ サイト全体のビジュアル統一性を評価: 配色・フォント・余白・写真のトーンに一貫性があるか
✓ ファーストビュー（トップページの最初の表示領域）がその企業/サービスの価値を瞬時に伝えられているか
✓ 写真・画像の質: ストックフォト頼りになっていないか、オリジナルの写真や図解が使われているか
✓ トンマナ（トーン＆マナー）: テキストの語調・表現がターゲットユーザーに合っているか
✓ 競合との差別化: このサイトならではの独自性・世界観が表現されているか
✓ 信頼感の演出: 実績・お客様の声・メディア掲載・認証バッジ等が適切に配置されているか
✓ ストーリーテリング: 会社の理念・開発背景・想いが伝わるコンテンツがあるか
✓ 各ページの統一感: 下層ページもトップページと同じブランド体験を提供できているか
✓ 改善提案では「何をどう変えるとブランドイメージが向上するか」をビジュアル面から具体的に提示
\n`;
    } else if (improvementFocus === 'ユーザービリティの向上') {
      improvementFocusLine += `【ユーザービリティ向上の重点ポイント】
※ 上記タスク1のデータのうち、特に「ページ別データ」「ページ分類別データ」のエンゲージメント率・平均滞在時間・直帰率を精査すること。
※ 「ページフロー」からユーザーが迷っている遷移・離脱が多いポイントを特定すること。
※ スクレイピングデータ（ALT属性の設定状況、見出し構造、読み込み速度、ページ構造）を根拠として活用すること。
※【重要】ユーザビリティに直接関係しない施策（SEO改善、コンテンツ追加、ブランディング等）は提案しないこと。あくまで「使いやすさ・見やすさ・アクセシビリティの改善」施策のみ出すこと。

✓ ページ別データを精査: 各ページのエンゲージメント率・平均滞在時間・直帰率を確認し、問題ページを特定
✓ ページ分類別データ（カテゴリ単位）で全体傾向を把握し、弱いカテゴリを特定
✓ ページフローを分析し、ユーザーが迷っている箇所・離脱が多い遷移を特定
✓ ナビゲーション構造: メニュー構成・パンくずリスト・サイト内検索の使いやすさを評価
✓ 表示速度: Core Web Vitals（LCP・CLS・INP）に基づく速度改善、画像最適化、不要スクリプトの削除
✓ モバイルUI: タップ領域のサイズ、フォントの可読性、横スクロールの有無、フォーム入力の快適さ
✓ アクセシビリティ: ALT属性の設定状況、コントラスト比、キーボード操作対応、スクリーンリーダー対応
✓ 情報設計: 見出し階層の適切さ、テキストの読みやすさ、重要情報の視認性
✓ 各施策に対して「どのページの、どの指標を、どう改善するか」を具体的に明示
\n`;
    }
  }
...
カテゴリー: （以下の定義に従い正確に分類すること）
  - acquisition: SEO・広告・SNS・外部流入など「サイトへの訪問者数を増やす」施策
  - content: テキスト・記事・コンテンツの追加・改善など「コンテンツの質や量を高める」施策
  - design: 配色・レイアウト・写真・ビジュアル・ブランド表現など「見た目やデザインを改善する」施策
  - feature: フォーム最適化・CTA改善・機能追加など「サイトの機能や仕組みを改善する」施策
  - other: 表示速度改善・alt属性設定・構造化データ・セキュリティなど「技術的・インフラ的な改善」施策
  ※ 注意: alt属性・読込速度・h1タグ・meta設定などの技術的改善は design ではなく other に分類すること
