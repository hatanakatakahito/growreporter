...
/**
 * 月次レポートを送信
 * Cloud Schedulerから毎月1日 9:00 JST に実行
 */
export async function sendMonthlyReportsHandler(event) {
  ...
  try {
    ...
    const ga4CurrentStart = format(lastMonthStart, 'yyyy-MM-dd');
    const ga4CurrentEnd = format(lastMonthEnd, 'yyyy-MM-dd');
    const ga4PrevStart = format(prevMonthStart, 'yyyy-MM-dd');
    const ga4PrevEnd = format(prevMonthEnd, 'yyyy-MM-dd');
