...
> **NOTE (2026-04-23 時点)**: V2 移行は完了済み。本ドキュメント内に登場する `sheetsManager.js` や `exportToSheets` などのスプレッドシート関連処理は v5.13.3 で完全に削除されました（Firestore の V2 タクソノミーが正の source of truth）。以下の記載は移行当時の歴史的な記録として残しています。
